...
public void LoadGoals()
{
    ...
    foreach (string line in lines)
    {
        ...
        // Determine the goal type and create the appropriate instance
        Goal goal;
        if (goalType == "ChecklistGoal")
        {
            ...
            goal = new ChecklistGoal(goalType, goalName, description, points, bonusPoints, targetCount, currentCount);
        }
        else if (goalType == "EternalGoal")
        {
            goal = new EternalGoal(goalType, goalName, description, points);
        }
        else
        {
            string status = goalData[3];
            goal = new SimpleGoal(goalType, goalName, description, points, status);
        }
...
{   
    ...
    // Verifica se o número da meta é válido
    if (goalNumber >= 1 && goalNumber <= goals.Count)
    {
        Goal goal = goals[goalNumber - 1]; 
        _index = goalNumber - 1;
        goal.RecordEvent(_index); // Passa o objeto goal como parâmetro para o método RecordEvent
    }
    else
    {
